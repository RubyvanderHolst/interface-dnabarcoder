...
def classify_blast(input_sequences_path, reference_path,
    ...
    for file in files:
        if file.endswith('.classified'):
            ...
            break
    html_classification_table, table_file_path = get_table_classification(result_file, task_id)
...
def get_table_classification(result_file, task_id):
    ...
    if check_results_generated(result_file) and result_file is not None:
        ...
        table_file_path_root = os.path.join(result_file.replace('.classified', '.only_classified'))
        table_file_path_js = os.path.join(media_url, 'results', task_id,
                                          os.path.basename(result_file).replace('.classified', '.only_classified'))
        df_classified.to_csv(table_file_path_root,
                             index=False,
                             encoding='utf-8',
                             sep='\t')
